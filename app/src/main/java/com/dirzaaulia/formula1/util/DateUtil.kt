...
fun findClosestFutureDate(dateStrings: List<Race>): Race? {
    ...
    return dateStrings
        .mapNotNull {
            try {
                LocalDateTime.parse(
                    "${it.schedule?.race?.date} ${it.schedule?.race?.time}",
                    formatter
                )
            } catch (e: Exception) {
                // Handle parsing errors if the format is wrong.
                println("Error parsing date: $it, error : ${e.message}")
                null
            }
        }
        .filter { it.isAfter(now) } // Filter out past dates
        .minOfOrNull { ChronoUnit.SECONDS.between(now, it) } // Find the minimum difference from now
        ?.let { minDifference ->
            dateStrings.firstOrNull {
                LocalDateTime.parse(
                    "${it.schedule?.race?.date} ${it.schedule?.race?.time}",
                    formatter
                ).isAfter(now) &&
