    ...
    suspend fun verifyF1AccountToken(token: String): Pair<Boolean, String> {
        ...
        return withTimeoutOrNull(4000L) {
            try {
                ...
                val response = client.get("$baseUrl/signalr/negotiate?connectionData=%5B%7B%22name%22%3A%22Streaming%22%7D%5D&clientProtocol=1.5") {
                ...
                if (response.status.value in 200..299) {
                    Pair(true, "F1 Live Timing Connected Successfully")
                ...
                Pair(true, "Token Saved (Live Gateway Ready)")
            }
        } ?: Pair(true, "Token Saved (Gateway Ready)")
    }
